...
public class MetadataDeleteFunctionHandler implements RequestStreamHandler, Constants {
	...
	public void handleRequest(InputStream inputStream, OutputStream outputStream, Context context) throws IOException
	{
		...
		logger.log("Suppression d'une fiche");
		...
		try (MongoClient mongoClient = new MongoClient(uri)) {
			...
			if (deleteOne.getDeletedCount() != 1) {
				...
				message ="incorrect deleted count - "+deleteOne.getDeletedCount()+ " - "+uuid;
				responseJson.put("exception", message);
			} else {
				result = true;
